...
def config(routers, h2, h3, h4, DHCPServer):
    ...
    routers['r1'].cmd('iptables -t nat -A POSTROUTING -s 192.168.1.0/26 -o r1-eth0 -j SNAT --to-source 140.113.0.30')
    routers['r1'].cmd('iptables -t nat -A POSTROUTING -s 192.168.1.64/26 -o r1-eth0 -j SNAT --to-source 140.113.0.40')
    routers['r1'].cmd('iptables -t nat -A PREROUTING -d 140.113.0.40 -p tcp --dport 80 -j DNAT --to-destination 192.168.1.66:8000')
