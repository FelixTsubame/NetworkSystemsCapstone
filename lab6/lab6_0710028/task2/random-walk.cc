...
void Movement (Ptr<Node> node, double stepsSize, double stepsTime, FILE *fp);
...
int main (int argc, char *argv[])
{
  ...
  boundary = GetBoundary(fp);

  Simulator::Schedule (Seconds (stepsTime), &Movement, wirelessNodes.Get (0), stepsSize, stepsTime, fp);
...
void Movement (Ptr<Node> node, double stepsSize, double stepsTime, FILE *fp)
{
  // [TODO]:
  MOVE step;
  step = GetSingleMovement(fp);

  Ptr<MobilityModel> mob = node->GetObject<MobilityModel>();
  Vector m_position = mob->GetPosition();

  m_position.x = m_position.x+(step.speed*cos(step.rad));
  m_position.y = m_position.y+(step.speed*sin(step.rad));
  if(m_position.x > boundary[0].x || m_position.x < boundary[1].x)
  {
    PrintIllegalMsg(step.index);
  }
  else if(m_position.y > boundary[0].y || m_position.y < boundary[1].y)
  {
    PrintIllegalMsg(step.index);
  }
  else
  ...
  CheckLinkState(node); 
  Simulator::Schedule (Seconds (stepsTime), &Movement, node, stepsSize, stepsTime, fp);
}
